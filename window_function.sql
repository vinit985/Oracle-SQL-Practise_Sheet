...
SELECT 
    ...
    AVG(sales) OVER (PARTITION BY productid ORDER BY orderdate ROWS Between current row and 1 following) AS RunningAvgSales
FROM orders;
select version();
-- Rank the orders based on their sales from highest to lowest
use salesdb;
-- ROW_NUMBER - Don't handle ties(duplicate)
select * from orders;
select 
   *,
...
from orders;
-- RANK FUNCTION -> Handles ties(Duplicate)
select 
   *,
   ROW_NUMBER() OVER(ORDER BY sales desc) as RowNumber,
   RANK() OVER(ORDER BY sales desc) as Rankk,
